...
└────────────────────────────────────────────────────────────────┘
  ...
  NVD / GHSA / OSV / Gitee     Anthropic / OpenAI-compat
  GitHub / Maven
```
...
| `jvuln-collector` | Stage 1：NVD/GHSA/OSV/Gitee/Maven 情报采集 |
| `jvuln-patcher` | Stage 2：修复 commit 定位、Maven source diff、unified diff 提取 |
...
数据源优先级：NVD → GHSA → OSV → Gitee，结果合并去重。
...
补充规则：
- `NvdSource` 会从 `cpeMatch` 中提取 Maven 坐标和 `versionEndIncluding/versionEndExcluding`。
- `GiteeSource` 会按 CVE 检索公开 issue，为国内托管项目补充 `sourceRepo` 和参考链接。
- `IntelligenceStage` 会从参考链接 URL 和公告描述中补 `sourceRepo`、`artifactId`、`affectedTo`。
```
...
- 若 GitHub 侧没有 commit 线索，但 Maven Central 可用，Stage 2 仍可独立生成补丁 diff。
...
`POST /api/config/llm/{id}/test` 使用统一 LLM 请求参数验证端点可达和 API Key 有效性，并返回模型与 token 使用信息。
